...
dbc.Row([
    dbc.Col([
        dbc.Card([
        html.Legend("Filtrar Lançamento", className='card-tile'),
...
def graph1_show(data_despesa, data_receita, despesa, receita):
    ...
    df_rs = df_receitas.groupby("Data").sum().rename(columns={"Valor": "Receita"})

    df_acum = df_ds.join(df_rs, how="outer").fillna(0)
...
def graph2_show(data_despesa, data_receita, receita, despesa, start_date, end_date):
    df_ds = pd.DataFrame(data_despesa)
    df_rs = pd.DataFrame(data_receita)

    df_ds["Output"] = "Despesas" 
    df_rs["Output"] = "Receitas"
    df_final = pd.concat([df_ds, df_rs])
